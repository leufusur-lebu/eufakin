<div class="p-6 space-y-4">
    <div class="flex items-center justify-between">
        <flux:heading size="xl">Tratamientos kinesiología</flux:heading>
        <flux:button href="{{ route('admin.kine.treatments.create') }}" variant="primary" icon="plus" wire:navigate>Nuevo</flux:button>
    </div>

    @if (session('success')) <div class="rounded bg-green-100 p-3 text-green-700">{{ session('success') }}</div> @endif

    <div class="flex gap-3">
        <flux:input placeholder="Buscar diagnóstico..." wire:model.live.debounce.400ms="search" icon="magnifying-glass" class="flex-1" />
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y dark:divide-zinc-700">
                @forelse ($treatments as $t)
                    <tr>
                        <td class="px-3 py-2">{{ $t->kineProfile?->person?->full_name }}</td>
                        <td class="px-3 py-2">{{ $t->diagnostico }}</td>
                        <td class="px-3 py-2">{{ $t->professional?->full_name ?? '—' }}</td>
                        ...
                        <td class="px-3 py-2 text-right">
                            <flux:button size="sm" href="{{ route('admin.kine.treatments.edit', $t) }}" wire:navigate>Editar</flux:button>
