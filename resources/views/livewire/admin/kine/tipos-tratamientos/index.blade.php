<div class="p-6 space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
            <flux:heading size="xl">Catálogo de protocolos kine</flux:heading>
            <flux:text class="text-zinc-500">Servicios kinesiológicos con plantilla de sesiones lista para aplicar a un paciente</flux:text>
        </div>
        <div class="flex gap-2">
            <flux:button wire:click="openCategories" variant="ghost" icon="adjustments-horizontal">Categorías</flux:button>
            <flux:button href="{{ route('admin.kine.tipos-tratamientos.create') }}" variant="primary" icon="plus" wire:navigate>Nuevo protocolo</flux:button>
        </div>
    </div>
            ...
            <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                @foreach ($items as $t)
                    <div class="group relative overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm transition hover:shadow-md dark:border-zinc-700 dark:bg-zinc-900 {{ !$t->activo ? 'opacity-60' : '' }}">
                        <div class="h-1.5" style="background: {{ $t->color ?? '#0ea5e9' }}"></div>
                        <div class="p-4">
                            ...
                            <div class="mt-3 grid grid-cols-3 gap-2 rounded-lg bg-sky-50/60 p-2 text-center text-xs dark:bg-sky-950/20">
                                <div>
                                    <div class="font-bold text-sky-700 dark:text-sky-300">{{ $t->sesiones_recomendadas ?? 1 }}</div>
                                    <div class="text-[10px] text-zinc-500">sesiones</div>
                                </div>
                                <div>
                                    <div class="font-bold text-sky-700 dark:text-sky-300">{{ $t->intervalo_dias ?? 3 }}d</div>
                                    <div class="text-[10px] text-zinc-500">intervalo</div>
                                </div>
                                <div>
                                    <div class="font-bold text-sky-700 dark:text-sky-300">{{ $t->duracion_minutos }}'</div>
                            ...
                            <div class="mt-4 flex gap-1">
                                <flux:button href="{{ route('admin.kine.protocols.apply', $t) }}" wire:navigate variant="primary" size="sm" icon="sparkles" class="flex-1">Aplicar</flux:button>
                                <flux:button href="{{ route('admin.kine.tipos-tratamientos.edit', $t) }}" wire:navigate variant="ghost" size="sm" icon="pencil" />
    ...
    @empty
        <div class="rounded-xl border border-dashed border-zinc-300 p-12 text-center dark:border-zinc-700">
            ...
            <flux:button href="{{ route('admin.kine.tipos-tratamientos.create') }}" variant="primary" size="sm" class="mt-3" wire:navigate>Crear primer protocolo</flux:button>
        </div>
    @endforelse
