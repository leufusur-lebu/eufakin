<div class="mx-auto max-w-4xl p-6">
    <div class="mb-4 flex items-center gap-2">
        <flux:button href="{{ route('admin.kine.tipos-tratamientos.index') }}" size="sm" variant="subtle" icon="arrow-left" wire:navigate>Volver</flux:button>
        <flux:heading size="xl">Nuevo protocolo kine</flux:heading>
    </div>
    ...
    <form wire:submit="save" class="space-y-6">
        <div class="rounded-xl border border-zinc-200 bg-white p-5 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <div class="grid gap-4 md:grid-cols-2">
                <flux:input wire:model="nombre" label="Nombre" placeholder="Ej. Rehabilitación rotuliana" required class="md:col-span-2" />
        ...
        <div class="rounded-xl border border-sky-200 bg-sky-50/40 p-5 dark:border-sky-900 dark:bg-sky-950/20">
            <div class="mb-4 flex items-center gap-2">
                <flux:icon.sparkles class="size-5 text-sky-600" />
                <h3 class="text-sm font-semibold uppercase tracking-wide text-sky-700 dark:text-sky-300">Plantilla de protocolo</h3>
            </div>
            ...
            <div class="grid gap-4 md:grid-cols-3">
                <flux:input wire:model="sesiones_recomendadas" type="number" min="1" max="60" label="Sesiones recomendadas" required />
            ...
            <div class="mt-3 rounded-lg bg-white p-3 text-xs dark:bg-zinc-900">
                ...
                <strong class="text-sky-600">${{ number_format(($precio_base ?? 0) * ($sesiones_recomendadas ?? 1), 0, ',', '.') }}</strong>
                ({{ $sesiones_recomendadas ?? 1 }} sesiones × ${{ number_format($precio_base ?? 0, 0, ',', '.') }})
            </div>
            <flux:textarea wire:model="protocolo" label="Detalle del protocolo" rows="3" class="mt-4" placeholder="Ejercicios por fase, progresión esperada, criterios de alta..." />
        </div>

        <div class="rounded-xl border border-zinc-200 bg-white p-5 dark:border-zinc-700 dark:bg-zinc-900">
            <h3 class="mb-4 text-sm font-semibold uppercase tracking-wide text-zinc-500">Información clínica</h3>
            <div class="space-y-4">
                <flux:textarea wire:model="materiales_requeridos" label="Materiales requeridos" rows="2" placeholder="Bandas, theraband, electroterapia, hielo..." />
                <flux:textarea wire:model="contraindicaciones" label="Contraindicaciones" rows="2" placeholder="Fracturas inestables, infecciones, etc..." />
            </div>
        </div>
        ...
        <div class="flex items-center justify-between rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <div class="flex gap-2">
                <flux:button href="{{ route('admin.kine.tipos-tratamientos.index') }}" variant="subtle" wire:navigate>Cancelar</flux:button>
